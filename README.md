...
## Самый простой запуск (через браузер, в 1 команду)
...
- Регистрация не обязательна (guest mode включен).
- `ANTHROPIC_API_KEY` может быть пустым (`""`) — AI вернет fallback-ответы.
...
## Ручной локальный запуск (без Docker web)
